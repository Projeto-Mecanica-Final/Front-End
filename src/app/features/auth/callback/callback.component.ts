  ...
  template: `
    <div class="callback-container">
      @if (!hasError()) {
        ...
        <p class="text-muted small">Aguarde enquanto validamos suas credenciais</p>
      } @else {
...
export class CallbackComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      const message = params['message']; // Pode vir do backend
      
      if (error || message) {
        // Se veio um erro na URL
        console.error('❌ Erro recebido na URL:', error);
        this.hasError.set(true);
        
        // Ignora a mensagem do backend, usa a nossa
        const errorType = error || 'google_auth_failed';
        this.errorMessage.set(this.getErrorMessage(errorType));
        
        // Redireciona para login com tipo apropriado
        const tipoMensagem = (errorType === 'user_not_registered' || errorType === 'access_denied') ? 'info' : 'error';
  ...
  private getErrorMessage(error: string): string {
    const messages: Record<string, string> = {
      'user_not_registered': 'Usuário não cadastrado no sistema. Entre em contato com o administrador.',
